...
function addChemthrowerDamageJEI(liquid as ILiquidStack, damage as float) {
  JEI.createJeiRecipe(CHEM_ID).addInput(liquid).addOutput(damage > 0 ? <engineersdecor:sign_defense>.withDisplayName('§cDamage') * damage : null).build();
}
function addChemthrowerFlammableJEI(liquid as ILiquidStack, damage as float) {
  JEI.createJeiRecipe(CHEM_ID).addInput(liquid).addOutput(damage > 0 ? <engineersdecor:sign_defense>.withDisplayName('§cDamage') * damage : null).addOutput(<minecraft:flint_and_steel>.withDisplayName('§6Flammable')).build();
}
