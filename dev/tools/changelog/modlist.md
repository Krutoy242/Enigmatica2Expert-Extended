...
**]({{padEnd now.cf2Addon.links.websiteUrl 74 post=')'~}}
 | <nobr>`{{replace was.installedFile.fileName '.jar' ''}}`</nobr><br><nobr>`{{replace now.installedFile.fileName '.jar' ''}}`</nobr>{{#if now.changelog}}<br><details><summary>Changelog {{now.changelogArrows}}</summary>{{{stripNewlines now.changelog}}}</details>{{/if}}
{{/each}}
-----------
